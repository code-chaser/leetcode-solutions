...
public:
    int maxSubArray(vector<int>& nums) {
        ...
        if (segments.size() == 1) return maxElement;
        int maxSum = maxElement;
        for(int i = 0, j = segments.size(); (i < segments.size()) && (j > 0) && (j > i); i += 2, j -= 2){
            while((i < (segments.size() - 1)) && ((segments[i] + segments[i+1]) <= 0)) i+=2;
            while((j > i) && ((segments[j-1] + segments[j-2]) <= 0)) j-=2;
            if((i < segments.size()) && (j > 0) && (j > i))
